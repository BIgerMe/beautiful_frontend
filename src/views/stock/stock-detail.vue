    ...
    <div class="app-container">
      <div class="filter-container">
        <el-descriptions title="基础信息" border  :column="4" size="middle">
          <el-descriptions-item label-class-name="my-label" content-class-name="my-content" label="名称" :span="3">{{ stock_basic.name+'（'+stock_basic.fullname+'）' }}</el-descriptions-item>
          <el-descriptions-item label-class-name="my-label" content-class-name="my-content" label="代码">{{ stock_basic.ts_code }}</el-descriptions-item>
        ...
        <el-card v-if="share_float.length > 0">
          ...
          <el-timeline>
            ...
            >
              ...
              解禁比例：{{value.float_ratio + '%'}} ，
              流通股份：{{parseFloat(parseFloat(value.float_share)/10000).toFixed(2)+'万股'}} ，
              持股方：{{value.holder_name}}
            </el-timeline-item>
          </el-timeline>
        </el-card>
        <br>
        <el-card v-if="repurchase.length > 0" style="max-height: 300px;overflow-y: auto">
            <div slot="header" class="clearfix">
              <span>逆回购</span>
            </div>
            <el-timeline>
              <el-timeline-item
                v-for="(value, index) in repurchase"
                :key="index"
              >
                <el-tag>{{value.proc}}</el-tag>
                【公告日期】{{value.ann_date}} <!--【截止日期】{{value.end_date}} 【过期日期】{{value.exp_date}}-->
                回购数量：{{parseFloat(parseFloat(value.vol)/10000).toFixed(2)+'万股'}} ，
                回购金额：{{parseFloat(parseFloat(value.amount)/10000).toFixed(2)+'万元'}} ，
                回购最高价 ：{{value.high_limit}}
                回购最低价 ：{{value.low_limit}}
              </el-timeline-item>
            </el-timeline>
          </el-card>
        <br>
